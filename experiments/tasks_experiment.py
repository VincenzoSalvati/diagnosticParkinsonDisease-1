...
@file tasks_experiment.py
PURPOSE OF THE FILE: Performs Tasks experiments.
"""
...
def get_path_performances_tasks_default_hyperparameters(config):
    """Returns path where (tasks) performances of models using default hyperparameters has to be saved

    Args:
        config (dict): The instance of config.yml

    Return:
        string: The path where (tasks) performances of models using default hyperparameters has to be saved
    """
    return config['tasks_default_hyperparameters']['performances']


def tasks_experiment(config, dataset_tasks, random_state):
    """Performs models' performances of tasks using default hyperparameters and save them into a csv
    ...
    for num_task in range(0, len(dataset_tasks)):
        ...
        if not os.path.exists(
            ...
            print("Selected features on task " + str(num_task) +
        ...
        for model_info in tqdm(models_info, "Training all models on task (Tasks Experiment)" + str(num_task)):
            ...
            table_performances = perform_experiment_selected_features_from(feature_selection_configurations, datasets,
                                                                           labels,
                                                                           name_model, random_state,
                                                                           table_performances)

        # Saves results
        write_csv_in(
            get_path_performances_tasks_default_hyperparameters(config) + str(num_task) + "_random_state_" +
